...
class MarketGlobalTvlMetricViewController: MarketListViewController {
    ...
    init(listViewModel: MarketListViewModel, headerViewModel: MarketTvlSortHeaderViewModel, chartViewModel: MetricChartViewModel, configuration: ChartConfiguration) {
    ...
    override func viewDidLoad() {
        ...
        chartIntervalAndSelectedRateCell.onSelectInterval = { [weak self] in self?.onSelectInterval(index: $0) }
    ...
    private func onSelectInterval(index: Int) {
        chartViewModel.onSelectType(at: index)
        headerViewModel.onSelectPriceChangeField(index: index)
    }
...
extension MarketGlobalTvlMetricViewController {
