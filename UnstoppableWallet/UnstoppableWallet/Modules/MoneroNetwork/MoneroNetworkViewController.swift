...
class MoneroNetworkViewController: ThemeViewController {
    private let viewModel: MoneroNetworkViewModel
    private let disposeBag = DisposeBag()

    private let tableView = SectionsTableView(style: .grouped)
    private let iconImageView = UIImageView()

    private var defaultViewItems = [MoneroNetworkViewModel.ViewItem]()
    private var customViewItems = [MoneroNetworkViewModel.ViewItem]()
    private var isLoaded = false

    init(viewModel: MoneroNetworkViewModel) {
    ...
    private func openAddNew() {
        stat(page: .blockchainSettingsMonero, event: .openBlockchainSettingsMoneroAdd(chainUid: viewModel.blockchainType.uid))
...
extension MoneroNetworkViewController: SectionsDataSource {
    ...
    private func row(id: String, viewItem: MoneroNetworkViewModel.ViewItem, rowActionProvider: (() -> [RowAction])? = nil, isFirst: Bool, isLast: Bool, action: @escaping () -> Void) -> RowProtocol {
    ...
    func buildSections() -> [SectionProtocol] {
        var sections: [SectionProtocol] = [
            Section(
                ...
                rows: [
                    tableView.descriptionRow(id: "description-row", text: "monero_network.description".localized, font: .subhead2, textColor: .themeGray),
                ]
            ),
            Section(
                id: "default",
        ...
        if !customViewItems.isEmpty {
            sections.append(
                Section(
                    id: "custom",
                    headerState: tableView.sectionHeader(text: "monero_network.added".localized),
            ...
            Section(
                ...
                rows: [
                    tableView.universalRow48(
                        ...
                        title: .body("monero_network.add_new".localized, color: .themeJacob),
                        autoDeselect: true,
                        isFirst: true,
                        isLast: true,
                        action: { [weak self] in
