...
enum FeeSettings {
    static func feeAmount(feeToken: Token, currency: Currency, feeTokenRate: Decimal?, loading: Bool, gasLimit: Int?, gasPrice: GasPrice?) -> (String?, String?, String?) {
        guard !loading else {
            return (nil, nil, nil)
        }

        guard let gasLimit, let gasPrice else {
            return ("n/a".localized, "n/a".localized, "n/a".localized)
        }
        ...
        guard let feeTokenRate else {
            return (coinValueString, "n/a".localized, gasLimit.description)
        }
        ...
        return (coinValueString, currencyValueString, gasLimit.description)
    }
