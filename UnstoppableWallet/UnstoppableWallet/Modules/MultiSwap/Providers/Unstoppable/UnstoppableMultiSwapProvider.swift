...
class UnstoppableMultiSwapProvider: IMultiSwapProvider {
    ...
    private func quoteSwapBestRoute(
        ...
        if includeTx {
            sourceAddress = sendingAddress(token: tokenIn)
        } else {
        ...
        let quoteResponse = try await provider.quote(
            request: .init(
                ...
                providers: assetIn.providers.intersection(assetOut.providers),
        ...
        guard let bestRoute = quoteResponse.routes.max(by: { ($0.expectedBuyAmount ?? 0) < ($1.expectedBuyAmount ?? 0) }) else {
    ...
    private func sendingAddress(token: Token) -> String? {
        ...
        if blockchainType.isEvm || blockchainType == .tron || blockchainType == .zcash {
