...
//            hideLoading()//todo need error design
    ...
    private func updateAlertBarItem(alertMode: ChartPriceAlertMode) {
        switch alertMode {
        case .on:
            let image = UIImage(named: "notification_active_24")?.tinted(with: .themeJacob)?.withRenderingMode(.alwaysOriginal)
            navigationItem.rightBarButtonItem = UIBarButtonItem(image: image, style: .plain, target: self, action: #selector(onAlertTap))
        case .off:
            let image = UIImage(named: "notification_24")?.tinted(with: .themeGray)?.withRenderingMode(.alwaysOriginal)
