...
class TransactionsViewItemFactoryNew {
    ...
    private func type(value: AppValue, condition: Bool = true, _ trueType: TransactionsViewModelNew.ValueType, _ falseType: TransactionsViewModelNew.ValueType? = nil) -> TransactionsViewModelNew.ValueType {
    ...
    private func singleValueSecondaryValue(value: AppValue, currencyValue: CurrencyValue?, nftMetadata: [NftUid: NftAssetBriefMetadata]) -> TransactionsViewModelNew.Value? {
        switch value.kind {
        case let .nft(nftUid, tokenName, _):
            let text = nftMetadata[nftUid]?.name ?? tokenName.map { "\($0) #\(nftUid.tokenId)" } ?? "#\(nftUid.tokenId)"
            return TransactionsViewModelNew.Value(text: text, type: .secondary)
        default:
            return currencyValue.map { TransactionsViewModelNew.Value(text: currencyString(from: $0), type: .secondary) }
        }
    }

    private func singleValueIconType(source: TransactionSource, kind: AppValue.Kind, nftMetadata: [NftUid: NftAssetBriefMetadata] = [:]) -> TransactionsViewModelNew.IconType {
        switch kind {
    ...
    private func doubleValueIconType(source: TransactionSource, primaryValue: AppValue?, secondaryValue: AppValue?, nftMetadata: [NftUid: NftAssetBriefMetadata] = [:]) -> TransactionsViewModelNew.IconType {
    ...
    private func iconType(source: TransactionSource, incomingValues: [AppValue], outgoingValues: [AppValue], nftMetadata: [NftUid: NftAssetBriefMetadata]) -> TransactionsViewModelNew.IconType {
    ...
    private func values(incomingValues: [AppValue], outgoingValues: [AppValue], currencyValue: CurrencyValue?, nftMetadata: [NftUid: NftAssetBriefMetadata]) -> (TransactionsViewModelNew.Value?, TransactionsViewModelNew.Value?) {
        var primaryValue: TransactionsViewModelNew.Value?
        var secondaryValue: TransactionsViewModelNew.Value?

        if incomingValues.count == 1, outgoingValues.isEmpty {
            let incomingValue = incomingValues[0]
            primaryValue = TransactionsViewModelNew.Value(text: coinString(from: incomingValue), type: type(value: incomingValue, .incoming))
            ...
            primaryValue = TransactionsViewModelNew.Value(text: coinString(from: outgoingValue), type: type(value: outgoingValue, .outgoing))
            ...
            primaryValue = TransactionsViewModelNew.Value(text: coinCodes, type: .incoming)
            secondaryValue = TransactionsViewModelNew.Value(text: "transactions.multiple".localized, type: .secondary)
        } else if incomingValues.isEmpty, !outgoingValues.isEmpty {
            let coinCodes = outgoingValues.map(\.code).joined(separator: ", ")
            primaryValue = TransactionsViewModelNew.Value(text: coinCodes, type: .outgoing)
            secondaryValue = TransactionsViewModelNew.Value(text: "transactions.multiple".localized, type: .secondary)
        } else {
            if incomingValues.count == 1 {
                primaryValue = TransactionsViewModelNew.Value(text: coinString(from: incomingValues[0]), type: type(value: incomingValues[0], .incoming))
            } else {
                let incomingCoinCodes = incomingValues.map(\.code).joined(separator: ", ")
                primaryValue = TransactionsViewModelNew.Value(text: incomingCoinCodes, type: .incoming)
            }
            if outgoingValues.count == 1 {
                secondaryValue = TransactionsViewModelNew.Value(text: coinString(from: outgoingValues[0]), type: type(value: outgoingValues[0], .outgoing))
            } else {
                let outgoingCoinCodes = outgoingValues.map(\.code).joined(separator: ", ")
                secondaryValue = TransactionsViewModelNew.Value(text: outgoingCoinCodes, type: .outgoing)
            }
        }
    ...
    func viewItem(item: TransactionsViewModelNew.Item, balanceHidden: Bool) -> TransactionsViewModelNew.ViewItem {
        var iconType: TransactionsViewModelNew.IconType
        let title: String
        let subTitle: String
        var primaryValue: TransactionsViewModelNew.Value?
        var secondaryValue: TransactionsViewModelNew.Value?
        ...
        case let record as EvmIncomingTransactionRecord:
            ...
            primaryValue = TransactionsViewModelNew.Value(text: coinString(from: record.value), type: type(value: record.value, .incoming))

            if let currencyValue = item.currencyValue {
                secondaryValue = TransactionsViewModelNew.Value(text: currencyString(from: currencyValue), type: .secondary)
            }
        ...
        case let record as EvmOutgoingTransactionRecord:
            ...
            primaryValue = TransactionsViewModelNew.Value(text: coinString(from: record.value, signType: record.sentToSelf ? .never : .always), type: type(value: record.value, condition: record.sentToSelf, .neutral, .outgoing))
        ...
        case let record as SwapTransactionRecord:
            ...
            if let valueOut = record.valueOut {
                primaryValue = TransactionsViewModelNew.Value(text: coinString(from: valueOut), type: type(value: valueOut, condition: record.recipient != nil, .secondary, .incoming))
            }

            secondaryValue = TransactionsViewModelNew.Value(text: coinString(from: record.valueIn), type: type(value: record.valueIn, .outgoing))
        ...
        case let record as UnknownSwapTransactionRecord:
            ...
            if let valueOut = record.valueOut {
                primaryValue = TransactionsViewModelNew.Value(text: coinString(from: valueOut), type: type(value: valueOut, .incoming))
            }
            if let valueIn = record.valueIn {
                secondaryValue = TransactionsViewModelNew.Value(text: coinString(from: valueIn), type: type(value: valueIn, .outgoing))
            }
        ...
        case let record as ApproveTransactionRecord:
            ...
            if record.value.isMaxValue {
                primaryValue = TransactionsViewModelNew.Value(text: "∞ \(record.value.code)", type: .neutral)
                secondaryValue = TransactionsViewModelNew.Value(text: "transactions.value.unlimited".localized, type: .secondary)
            } else {
                primaryValue = TransactionsViewModelNew.Value(text: coinString(from: record.value, signType: .never), type: .neutral)
        ...
        case let record as TonTransactionRecord:
            if record.actions.count == 1, let action = record.actions.first {
                ...
                case let .send(value, to, _sentToSelf, _):
                    ...
                    primaryValue = TransactionsViewModelNew.Value(text: coinString(from: value, signType: _sentToSelf ? .never : .always), type: type(value: value, condition: _sentToSelf, .neutral, .outgoing))
                ...
                case let .receive(value, from, _):
                    ...
                    primaryValue = TransactionsViewModelNew.Value(text: coinString(from: value), type: type(value: value, .incoming))
                ...
                case let .burn(value):
                    ...
                    subTitle = value.name
                    primaryValue = TransactionsViewModelNew.Value(text: coinString(from: value), type: type(value: value, .outgoing))
        ...
        case let record as StellarTransactionRecord:
            ...
            case let .accountCreated(startingBalance, funder):
                ...
                primaryValue = TransactionsViewModelNew.Value(text: coinString(from: startingBalance), type: type(value: startingBalance, .incoming))
            ...
            case let .accountFunded(startingBalance, account):
                ...
                primaryValue = TransactionsViewModelNew.Value(text: coinString(from: startingBalance), type: type(value: startingBalance, .outgoing))
            ...
            case let .changeTrust(value, _, trustee, _):
                ...
                if value.isMaxValue {
                    primaryValue = TransactionsViewModelNew.Value(text: "∞ \(value.code)", type: .neutral)
                    secondaryValue = TransactionsViewModelNew.Value(text: "transactions.value.unlimited".localized, type: .secondary)
                } else {
                    primaryValue = TransactionsViewModelNew.Value(text: coinString(from: value, signType: .never), type: .neutral)
        ...
        if balanceHidden {
            primaryValue = TransactionsViewModelNew.Value(text: BalanceHiddenManager.placeholder, type: primaryValue?.type ?? .neutral)
            secondaryValue = TransactionsViewModelNew.Value(text: BalanceHiddenManager.placeholder, type: secondaryValue?.type ?? .neutral)
        }

        return TransactionsViewModelNew.ViewItem(
            id: item.record.uid,
