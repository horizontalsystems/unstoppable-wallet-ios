...
class RestoreSelectService {
    ...
    init(accountType: AccountType, accountFactory: AccountFactory, accountManager: AccountManager, walletManager: WalletManager, testNetManager: TestNetManager, evmAccountRestoreStateManager: EvmAccountRestoreStateManager, marketKit: MarketKit.Kit, enableCoinService: EnableCoinService) {
    ...
    private func hasSettings(token: Token) -> Bool {
        !token.blockchainType.coinSettingTypes(accountOrigin: .restored).isEmpty
    }
...
extension RestoreSelectService {
    ...
    func enable(blockchainUid: String) {
        ...
        enableCoinService.enable(fullCoin: token.fullCoin, accountType: accountType, accountOrigin: .restored)
    }
    ...
    func configure(blockchainUid: String) {
        ...
        enableCoinService.configure(fullCoin: token.fullCoin, accountType: accountType, accountOrigin: .restored, configuredTokens: enabledConfiguredTokens.filter { $0.token == token })
    }
