    ...
    private func bindActions(cell: BalanceButtonsCell) {
        ...
        case let .wallet(wallet):
            cell.actions[.send] = { [weak self] in
                let module = App.shared.localStorage.newSendEnabled ? PreSendView(wallet: wallet).toNavigationViewController() : SendModule.controller(wallet: wallet).map { ThemeNavigationController(rootViewController: $0) }
