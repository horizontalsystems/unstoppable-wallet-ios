...
class TwitterText {
    ...
    static func entities(in text: String) -> [Entity] {
        ...
        let hashtags = hashtags(in: text, with: urls)
        results.append(contentsOf: hashtags)

        let symbols = symbols(in: text, with: urls)
    ...
    static func urls(in text: String) -> [Entity] {
        ...
        while true {
            ...
            guard let urlResult = validURLRegexp.firstMatch(in: text, options: [.withoutAnchoringBounds], range: NSMakeRange(position, len - position)) else {
                break
            }
            ...
            if urlProtocol == nil || urlProtocol?.count == 0 {
                ...
                if let set = preceding?.rangeOfCharacter(from: invalidURLWithoutProtocolPrecedingCharSet, options: [.backwards, .anchored]) {
            ...
            if let url {
                tcoResult = validTCOURLRegexp.firstMatch(in: url, options: [], range: NSMakeRange(0, url.utf16.count))
            } else {
                tcoResult = nil
            }

            if let tcoResult, tcoResult.numberOfRanges >= 2 {
    ...
    static func hashtags(in text: String, checkingURLOverlap: Bool) -> [Entity] {
        ...
        return hashtags(in: text, with: urls)
    }
    ...
    static func hashtags(in text: String, with urlEntities: [Entity]) -> [Entity] {
        ...
        while true {
            let matchResult = validHashtagRegexp.firstMatch(in: text, options: [.withoutAnchoringBounds], range: NSMakeRange(position, len - position))
            ...
            if matchOk {
                ...
                if afterStart < len {
                    let endMatchRange = endHashtagRegexp.rangeOfFirstMatch(in: text, options: [], range: NSMakeRange(afterStart, len - afterStart))
    ...
    static func symbols(in text: String, with urlEntities: [Entity]) -> [Entity] {
        ...
        while true {
            let matchResult = validSymbolRegexp.firstMatch(in: text, options: [.withoutAnchoringBounds], range: NSMakeRange(position, len - position))
    ...
    static func mentionedScreenNames(in text: String) -> [Entity] {
        ...
        let mentionsOrLists = mentionsOrLists(in: text)
        var results: [Entity] = []
    ...
    static func mentionsOrLists(in text: String) -> [Entity] {
        ...
        while true {
            let matchResult = validMentionOrListRegexp.firstMatch(in: text, options: [.withoutAnchoringBounds], range: NSMakeRange(position, len - position))
            ...
            let endMentionRange = endMentionRegexp.rangeOfFirstMatch(in: text, options: [], range: NSMakeRange(end, len - end))
    ...
    static func repliedScreenName(in text: String) -> Entity? {
        ...
        let len = text.utf16.count
        let matchResult = validReplyRegexp.firstMatch(in: text, options: [.withoutAnchoringBounds, .anchored], range: NSMakeRange(0, len))
        ...
        let endMentionRange = endMentionRegexp.rangeOfFirstMatch(in: text, options: [], range: NSMakeRange(replyEnd, len - replyEnd))
    ...
    static func tweetLength(text: String) -> Int {
        tweetLength(text: text, transformedURLLength: transformedURLLength)
    }
    ...
    static func tweetLength(text: String, transformedURLLength: Int) -> Int {
        ...
        if len > 0 {
            var buffer: [UniChar] = Array(repeating: UniChar(), count: len)
    ...
    static func remainingCharacterCount(text: String) -> Int {
        remainingCharacterCount(text: text, transformedURLLength: transformedURLLength)
    }

    static func remainingCharacterCount(text: String, transformedURLLength: Int) -> Int {
        maxTweetLengthLegacy - tweetLength(text: text, transformedURLLength: transformedURLLength)
    }

    // MARK: - Private Methods

    static let invalidCharacterRegexp = try! NSRegularExpression(pattern: Regexp.TWUInvalidCharactersPattern, options: .caseInsensitive)
    ...
    private static let invalidURLWithoutProtocolPrecedingCharSet: CharacterSet = .init(charactersIn: "-_./")
    ...
    var isEmoji: Bool {
        do {
            ...
            return matches.count == 1
                ...
                && NSMaxRange(matches[0].range) <= utf16.count
        } catch {
            return false
        }
    }
