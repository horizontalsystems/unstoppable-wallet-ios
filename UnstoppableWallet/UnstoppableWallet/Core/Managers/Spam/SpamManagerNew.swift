...
class SpamManagerNew {
    ...
    private func handleAdaptersReady() {
        ...
        logger?.log(level: .debug, message: "SMNew: adapters ready, count=\(transactionAdapterManager?.adapterMap.count ?? 0)")
    ...
    private func hasOutgoingEvents(record: TransactionRecord) -> Bool { //TODO: check valid with external contractCalls
        switch record {
    ...
    func isSpam(transactionHash: Data, record: TransactionRecord) -> Bool {
        ...
        queue.sync { [weak self] in
            ...
            result = self.performCheckSpam(transactionHash: transactionHash, record: record)
        }
        return result
    }
    ...
    /// Check SpamTransactionInfo and save result.
    /// Used from EvmTransactionConverter for checking during conversion.
    func checkIsSpam(spamInfo: SpamTransactionInfo) -> Bool {
        ...
        queue.sync { [weak self] in
            ...
            // Double-check after entering queue
            if let scanned = try? self.storage.findScanned(transactionHash: hashData) {
                result = scanned.isSpam
                return
            }
            
            let isSpam = self.evaluateSpam(spamInfo: spamInfo)
            let spamAddress = isSpam ? spamInfo.events.incoming.first?.address : nil
            ...
            try? self.storage.save(scannedTransaction: scanned)
