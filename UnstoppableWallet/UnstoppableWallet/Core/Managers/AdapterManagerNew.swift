...
class AdapterManagerNew {
    ...
    private let adaptersReadyRelay = PublishRelay<[WalletNew: IAdapter]>()

    private let queue = DispatchQueue(label: "io.horizontalsystems.unstoppable.adapter_manager", qos: .userInitiated)
    private var _adapterMap = [WalletNew: IAdapter]()

    init(adapterFactory: AdapterFactoryNew, walletManager: WalletManagerNew, ethereumKitManager: EvmKitManager, binanceSmartChainKitManager: EvmKitManager, initialSyncSettingsManager: InitialSyncSettingsManager) {
    ...
    private func initAdapters(wallets: [WalletNew]) {
...
//        })
    ...
    private func refreshAdapters(wallets: [WalletNew]) {
...
extension AdapterManagerNew {

    var adapterMap: [WalletNew: IAdapter] {
        queue.sync { _adapterMap }
    }

    var adaptersReadyObservable: Observable<[WalletNew: IAdapter]> {
        adaptersReadyRelay.asObservable()
    }

    func adapter(for wallet: WalletNew) -> IAdapter? {
    ...
    func balanceAdapter(for wallet: WalletNew) -> IBalanceAdapter? {
        queue.sync { _adapterMap[wallet] as? IBalanceAdapter }
    }

    func depositAdapter(for wallet: WalletNew) -> IDepositAdapter? {
