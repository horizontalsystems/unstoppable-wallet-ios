...
struct WCEvmSignMessageParserTests {
    private let parser = WCEvmSignMessageParser()
    private let address = WCTestFixtures.address
    ...
    private func parse(method: String, params: Any) throws -> WCEvmSignMessagePayload {
        let request = try WCTestFixtures.request(method: method, params: AnyCodable(any: params))
        let result = try parser.parse(request: request)
        return try #require(result as? WCEvmSignMessagePayload)
    }
    ...
    @Test func typedDataAsStringExtractsDomain() throws {
        ...
        #expect(payload.typedData != nil)
        #expect(payload.typedDataDomain == WCTypedDataDomain(chainId: 1, verifyingContract: "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"))
    ...
    @Test func missingAddressIsRejected() throws {
        let request = try WCTestFixtures.request(method: "personal_sign", params: AnyCodable(any: ["hello", "world"]))
        #expect(throws: WCEvmSignMessageParser.ParsingError.missingAddress) { try parser.parse(request: request) }
    }

    @Test func malformedTypedDataIsRejected() throws {
        let request = try WCTestFixtures.request(method: "eth_signTypedData_v4", params: AnyCodable(any: [address, "{not json"]))
        #expect(throws: WCEvmSignMessageParser.ParsingError.malformedParams) { try parser.parse(request: request) }
    }

    @Test func ignoresOtherMethodsAndNamespaces() throws {
        let tx = try WCTestFixtures.request(method: "eth_sendTransaction", params: AnyCodable(any: [address, "0x"]))
        let solana = try WCTestFixtures.request(method: "personal_sign", chainId: "solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp", params: AnyCodable(any: ["m", address]))
