...
struct WCEvmPermitVerifierTests {
    private let verifier = WCEvmPermitVerifier()
    ...
    private func permit2(amount: String, contract: String = WCEvmPermitVerifier.permit2Contract) -> String {
        """
    ...
    private func context(json: String) throws -> WCVerificationContext {
        let request = try WCTestFixtures.request(method: "eth_signTypedData_v4", params: AnyCodable(any: [WCTestFixtures.address, json]))
        let parsed = try parser.parse(request: request)
        let payload = try #require(parsed)
        return try WCTestFixtures.context(payload: payload)
    }
    ...
    @Test func ignoresPlainMessages() throws {
        let context = try WCTestFixtures.context(payload: WCStubRequestPayload.make(method: "personal_sign", kind: .signMessage))
