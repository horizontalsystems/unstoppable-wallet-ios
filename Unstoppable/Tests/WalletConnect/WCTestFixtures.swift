...
enum WCTestFixtures {
    ...
    static func payload(method: String = "eth_sendTransaction", kind: WCRequestPayload.Kind = .transaction, from: String? = address) throws -> WCRequestPayload {
        try WCRequestPayload(request: request(method: method), kind: kind, from: from)
    }

    static func context(payload: WCRequestPayload? = nil, verifyContext: VerifyContext? = nil, approvedAccounts: [WalletConnectUtils.Account] = []) throws -> WCVerificationContext {
        try WCVerificationContext(payload: payload ?? self.payload(), verifyContext: verifyContext, accountId: "account-1", approvedAccounts: approvedAccounts)
    }
}

final class WCSpySignClient: IWCSignClient {
...
final class WCStubRequestPayload: WCRequestPayload, IWCTypedDataRequest {
    var stubTo: String?
    var stubValue: BigUInt?
    var stubData: Data?
    var stubMessage: Data?
    var stubSwapInfo: WCSwapInfo?
    var stubDomain: WCTypedDataDomain?
    ...
    override var decodedSwapInfo: WCSwapInfo? { stubSwapInfo }
    var typedDataDomain: WCTypedDataDomain? { stubDomain }

    static func make(method: String = "eth_sendTransaction", chainId: String = "eip155:1", kind: WCRequestPayload.Kind = .transaction, from: String? = WCTestFixtures.address) throws -> WCStubRequestPayload {
        try WCStubRequestPayload(request: WCTestFixtures.request(method: method, chainId: chainId), kind: kind, from: from)
    }
}

final class WCStubSwapRouterProvider: IWCSwapRouterProvider {
    var routers = [String: String]()

    func routerAddress(provider _: WCSwapInfo.Provider, chainId: Blockchain) -> String? {
