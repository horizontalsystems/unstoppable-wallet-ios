...
struct WCRequestServiceTests {
    ...
    private var session: WCSessionInfo {
        WCSessionInfo(topic: WCTestFixtures.topic, accountId: "account-1", dAppName: "React App", approvedAccounts: [WCTestFixtures.approvedMainnet])
    }

    private func service() -> WCRequestService {
        WCRequestService(parsers: parsers, verifiers: verifiers, responder: WCResponder(signClient: client))
    }
    ...
    @Test func passingTransactionYieldsSendData() async throws {
        ...
        guard case let .walletConnect(inner, carried) = sendData else {
            Issue.record("expected walletConnect send data")
            return
        }
        #expect(inner != nil)
    ...
    @Test func signMessageAndDirectBranch() async throws {
        ...
        let signRequest = try WCTestFixtures.request(method: "personal_sign")
        let directRequest = try WCTestFixtures.request(method: "wallet_switchEthereumChain")
    ...
    @Test func unsupportedMethodIsRejectedWith5101() async throws {
        let request = try WCTestFixtures.request(method: "eth_unknown")
    ...
    @Test func verifiersReceiveSessionAndOriginContext() async throws {
        ...
        #expect(received.approvedAccounts == [WCTestFixtures.approvedMainnet])
...
private final class StubParser: IWCParser {
    ...
    private let kind: WCRequestPayload.Kind
    private let sendData: SendData?
    private let method: String
    private let error: Error?

    init(kind: WCRequestPayload.Kind, sendData: SendData?, method: String = "eth_sendTransaction", error: Error? = nil) {
        self.kind = kind
...
private final class StubPayload: WCRequestPayload {
    private let sendData: SendData?

    init(request: Request, kind: WCRequestPayload.Kind, sendData: SendData?) {
        self.sendData = sendData
        super.init(request: request, kind: kind, from: WCTestFixtures.address)
    }
...
private final class StubVerifier: IWCVerifier {
    ...
    private(set) var receivedContext: WCVerificationContext?

    init(verdict: WCVerificationVerdict) {
        self.verdict = verdict
    }

    func handles(_: WCVerificationContext) -> Bool { true }
