...
enum WCStellarFixtures {
    ...
    static func request(method: String = WCStellarTransactionPayload.submitMethod, chainId: String = "stellar:pubnet", params: [String: String]) throws -> Request {
        guard let blockchain = WalletConnectUtils.Blockchain(chainId) else {
            throw FixtureError.invalidChain
        }
        return try Request(topic: WCTestFixtures.topic, method: method, params: AnyCodable(params), chainId: blockchain)
    }
