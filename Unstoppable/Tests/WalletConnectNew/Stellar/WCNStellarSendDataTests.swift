...
struct WCNStellarSendDataTests {
    ...
    @Test func signDataIsSignOnlyThroughWrapper() throws {
        ...
        let parsed = WCNStellarTransactionParsed(request: request, xdr: envelope.xdr, sourceAccountId: envelope.sourceAccountId)
        let inner = WCNStellarSignData(xdr: envelope.xdr, transaction: envelope.transaction, sourceAccountId: envelope.sourceAccountId)
        let data = WCNSendData(inner: inner, request: WCNRequest(parsed: parsed, verdict: .pass, dAppName: "dApp"))
