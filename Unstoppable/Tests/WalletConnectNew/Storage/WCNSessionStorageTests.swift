...
struct WCNSessionStorageTests {
    @Test func roundTripsApprovedNamespaces() throws {
        ...
        try storage.save(session: WCNSessionRecord(topic: "t1", accountId: "a1", dAppName: "React App", namespaces: namespaces))
        ...
        #expect(decoded.accounts == [WCNTestFixtures.approvedMainnet, WCNTestFixtures.approvedOptimism])
    }

    @Test func filtersByAccountAndDeletes() throws {
        let storage = try WCNSessionFixtures.storage()
        let namespaces = try WCNSessionNamespaces(sessionNamespaces: WCNSessionFixtures.session(topic: "x").namespaces)
        try storage.save(session: WCNSessionRecord(topic: "t1", accountId: "a1", dAppName: "d1", namespaces: namespaces))
        try storage.save(session: WCNSessionRecord(topic: "t2", accountId: "a2", dAppName: "d2", namespaces: namespaces))
        try storage.save(session: WCNSessionRecord(topic: "t3", accountId: "a1", dAppName: "d3", namespaces: namespaces))
    ...
    @Test func topicIsUniqueAcrossAccounts() throws {
        ...
        try storage.save(session: WCNSessionRecord(topic: "t1", accountId: "a2", dAppName: "d", namespaces: namespaces))
