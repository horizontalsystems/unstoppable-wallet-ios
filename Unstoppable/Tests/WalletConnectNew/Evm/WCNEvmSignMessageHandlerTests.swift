...
struct WCNEvmSignMessageHandlerTests {
    ...
    private func handler(provider: IWCNEvmSignerProvider) -> WCNEvmSignMessageHandler {
        WCNEvmSignMessageHandler(signerProvider: provider, responder: WCNResponder(signClient: client))
    }

    private func request(method: String, params: [Any]) throws -> WCNRequest {
        let raw = try WCNTestFixtures.request(method: method, params: AnyCodable(any: params))
        let parsed = try parser.parse(request: raw)
        let payload = try #require(parsed)
        return WCNRequest(payload: payload, verdict: .pass, dAppName: "dApp")
    }
    ...
    @Test func typedDataUsesEip712Signing() async throws {
        ...
        let typed = try request(method: "eth_signTypedData_v4", params: [address, WCNEvmSignMessageParserTests.typedDataJson])
        let personal = try request(method: "personal_sign", params: [WCNEvmSignMessageParserTests.typedDataJson, address])
    ...
    @Test func missingSignerFails() async throws {
        let request = try request(method: "personal_sign", params: ["Sign in", EvmKit.Signer.address(privateKey: Self.privateKey).eip55])
        await #expect(throws: WCNEvmSignMessageHandler.SignError.self) {
...
private final class StubSignerProvider: IWCNEvmSignerProvider {
