...
struct WCNSolanaSignMessageTests {
    private let client = WCNSpySignClient()
    private let parser = WCNSolanaSignMessageParser()

    private func request(message: String = "Sign in to Solana dApp", pubkey: String = SolanaRawSigningFixtures.ours) throws -> Request {
        try WCNTestFixtures.request(method: WCNSolanaSignMessagePayload.method, chainId: "solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp", params: AnyCodable(any: ["message": HsCryptoKit.Base58.encode(Data(message.utf8)), "pubkey": pubkey]))
    }

    @Test func parsesBase58MessageAndPubkey() throws {
        let result = try parser.parse(request: request())
        let payload = try #require(result as? WCNSolanaSignMessagePayload)
    ...
    @Test func invalidPubkeyIsRejected() throws {
        ...
        #expect(throws: WCNSolanaSignMessageParser.ParsingError.malformedParams) { try parser.parse(request: request) }
    }
    ...
    @Test func signsAndAnswersBase58Signature() async throws {
        ...
        let handler = WCNSolanaSignMessageHandler(signerProvider: StubSignerProvider(signer: signer), responder: WCNResponder(signClient: client))

        try await handler.sign(request: WCNRequest(payload: payload, verdict: .pass, dAppName: "dApp"))
...
private final class StubSignerProvider: IWCNSolanaSignerProvider {
