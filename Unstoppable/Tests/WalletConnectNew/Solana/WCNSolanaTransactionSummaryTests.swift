...
struct WCNSolanaTransactionSummaryTests {
    ...
    @Test func decodesSystemTransfer() {
        let summary = WCNSolanaTransactionSummary(rawTransaction: SolanaRawSigningFixtures.partiallySigned)
    ...
    @Test func decodesSplTransferChecked() throws {
        ...
        let summary = WCNSolanaTransactionSummary(rawTransaction: raw)
    ...
    @Test func unknownProgramIsOpaque() {
        let summary = WCNSolanaTransactionSummary(rawTransaction: SolanaRawSigningFixtures.rawTransaction(signerKeys: [ours], programKey: Data(repeating: 0x22, count: 32)))
    ...
    @Test func malformedBytesAreOpaque() {
        let summary = WCNSolanaTransactionSummary(rawTransaction: Data([1, 2, 3]))
        #expect(summary.opaque)
    }
}
