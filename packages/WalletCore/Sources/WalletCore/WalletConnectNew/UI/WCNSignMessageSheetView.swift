...
struct WCNSignMessageSheetView: View {
    @StateObject private var viewModel: WCNSignMessageViewModel
    @Binding private var isPresented: Bool

    init(item: WCNRequestItem, isPresented: Binding<Bool>) {
    ...
    var body: some View {
        ThemeView(style: .list) {
            VStack(spacing: 0) {
                WCNDAppTitleView(iconUrl: viewModel.iconUrl, title: "wallet_connect.sign.request_title".localized, showGrabber: true)
                    ...
                    Cell(
                        ...
                        action: {
                            Coordinator.shared.present(type: .bottomSheet) { isPresented in
                                WCNMessageSheetView(text: viewModel.message, isPresented: isPresented)
                            }
                        }
                    )
                }
