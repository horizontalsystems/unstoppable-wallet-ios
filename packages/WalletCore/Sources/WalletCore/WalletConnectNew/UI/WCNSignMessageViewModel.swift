...
class WCNSignMessageViewModel: ObservableObject {
    ...
    init(item: WCNRequestItem) {
        ...
        var rows = [Row]()
        if let evm = payload as? WCNEvmSignMessagePayload, let typedData = evm.typedData {
        ...
        if let payload, let type = manager?.chainSupportRegistry.support(namespace: payload.chainId.namespace)?.blockchainType(chain: payload.chainId) {
            rows.append(Row(title: "wallet_connect.sign.network".localized, value: WCNBlockchainsView.name(type: type), copyable: false))
        }
        ...
        switch payload {
        case let evm as WCNEvmSignMessagePayload: message = evm.readableMessage
        case let solana as WCNSolanaSignMessagePayload: message = solana.readableMessage
        default: message = ""
        }
