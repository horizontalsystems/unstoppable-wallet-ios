...
class WCNSessionsViewModel: ObservableObject {
    struct Item: Identifiable {
        let session: WCNSessionItem
        let host: String
        let iconUrl: URL?
    ...
    struct RequestItem: Identifiable {
        let pending: WCNPendingRequestItem
        let label: String
    ...
    private func subscribe(kit: WCNKit) {
        WCNLog.log("sessions vm: subscribed to kit")
    ...
    private func sync(sessions: [WCNSessionItem], kit: WCNKit) {
        WCNLog.log("sessions vm: sync sessions=\(sessions.count)")
