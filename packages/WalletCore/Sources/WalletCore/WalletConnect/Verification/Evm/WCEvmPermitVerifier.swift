...
class WCEvmPermitVerifier: IWCVerifier {
    ...
    func handles(_ context: WCVerificationContext) -> Bool {
        (context.payload as? WCEvmSignMessagePayload)?.typedData != nil
    }

    func verify(_ context: WCVerificationContext) -> WCVerificationVerdict {
        guard let payload = context.payload as? WCEvmSignMessagePayload, let typedData = payload.typedData,
