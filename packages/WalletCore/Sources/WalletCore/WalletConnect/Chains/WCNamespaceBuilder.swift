...
class WCNamespaceBuilder {
    private let registry: WCChainSupportRegistry

    init(registry: WCChainSupportRegistry) {
        self.registry = registry
    }

    func proposals(required: [String: ProposalNamespace], optional: [String: ProposalNamespace]?, account: Account) -> [WCBlockchainProposal] {
        var proposals = [WCBlockchainProposal]()
    ...
    func validate(required: [String: ProposalNamespace], selected: [WCBlockchainProposal]) throws {
    ...
    func sessionNamespaces(selected: [WCBlockchainProposal]) -> [String: SessionNamespace] {
    ...
    private func append(key: String, namespace: ProposalNamespace, required: Bool, account: Account, into proposals: inout [WCBlockchainProposal]) {
        ...
        for chain in Self.chains(key: key, namespace: namespace) where supportedChains.contains(chain) {
            ...
            proposals.append(WCBlockchainProposal(chain: chain, account: caipAccount, methods: methods, events: events, required: required))
        }
    }
