...
class WCSolanaTransactionParser: IWCParser {
    ...
    func parse(request: Request) throws -> WCRequestPayload? {
        guard request.chainId.namespace == WCNamespace.solana,
              [WCSolanaTransactionPayload.signMethod, WCSolanaTransactionPayload.signAllMethod, WCSolanaTransactionPayload.signAndSendMethod].contains(request.method)
        else {
            return nil
        }

        let params = try? request.params.get(Params.self)
        let encoded = request.method == WCSolanaTransactionPayload.signAllMethod ? params?.transactions : params?.transaction.map { [$0] }
        ...
        guard encoded.count <= WCSolanaLimits.maxTransactionsPerRequest,
              encoded.allSatisfy({ $0.count <= WCSolanaLimits.maxTransactionBase64Length })
        else {
            throw ParsingError.malformedParams
        }
        ...
        return WCSolanaTransactionPayload(request: request, rawTransactions: rawTransactions, requiredSigners: requiredSigners, from: from)
    }
}

extension WCSolanaTransactionParser {
