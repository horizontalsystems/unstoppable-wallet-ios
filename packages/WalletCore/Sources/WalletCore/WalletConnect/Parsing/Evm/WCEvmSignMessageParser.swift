...
class WCEvmSignMessageParser: IWCParser {
    func parse(request: Request) throws -> WCRequestPayload? {
        guard request.chainId.namespace == WCNamespace.eip155 else {
            return nil
        }

        switch request.method {
        case WCEvmSignMessagePayload.ethSignMethod, WCEvmSignMessagePayload.personalSignMethod:
            let (address, message) = try Self.addressAndMessage(params: request.params)
            return WCEvmSignMessagePayload(request: request, address: address, message: message, typedData: nil)

        case let method where WCEvmSignMessagePayload.typedDataMethods.contains(method):
            ...
            return WCEvmSignMessagePayload(request: request, address: address, message: json, typedData: typedData)
...
extension WCEvmSignMessageParser {
