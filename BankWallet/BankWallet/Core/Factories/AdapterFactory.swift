class AdapterFactory: IAdapterFactory {
    ...
    func adapter(forCoin coin: Coin, authData: AuthData) -> IAdapter? {
        ...
        case .bitcoin:
            ...
            return try? BitcoinAdapter(coin: coin, authData: authData, syncMode: localStorage.syncMode, addressParser: addressParser, feeRateProvider: feeRateProvider, testMode: appConfigProvider.testMode)
        case .bitcoinCash:
            let addressParser = AddressParser(validScheme: "bitcoincash", removeScheme: false)
            return try? BitcoinCashAdapter(coin: coin, authData: authData, syncMode: localStorage.syncMode, addressParser: addressParser, feeRateProvider: feeRateProvider, testMode: appConfigProvider.testMode)
        case .dash:
            let addressParser = AddressParser(validScheme: "dash", removeScheme: true)
            return try? DashAdapter(coin: coin, authData: authData, syncMode: localStorage.syncMode, addressParser: addressParser, feeRateProvider: feeRateProvider, testMode: appConfigProvider.testMode)
        case .ethereum:
