import UIKit

class DepositTheme {
    static let collectionHeight: CGFloat = 300
    static let regularMargin: CGFloat = 16

    static let titleFont = UIFont.systemFont(ofSize: 17, weight: .bold)
    static var titleColor: UIColor { return .crypto_White_Black }

    static let qrCodeSideSize: CGFloat = 150
    static let qrCodeTopMargin: CGFloat = 16

    static let addressTitleTopMargin: CGFloat = 16
    static let addressTitleFont: UIFont = .systemFont(ofSize: 14)
    static var addressTitleColor: UIColor = .cryptoGray

    static let addressTopMargin: CGFloat = 8

    static let pagingHeight: CGFloat = 30
    static let pageIndicatorTintColor = UIColor.cryptoSteel20
    static let pagingDotsTopMargin: CGFloat = 23
    static let pagingDotsHeight: CGFloat = 7
    static let pageIndicatorSelectedTintColor = UIColor.cryptoGreen

    static let shareButtonItemHeight: CGFloat = 50 + 16 * 2
    static let shareButtonCornerRadius: CGFloat = 8
}
