...
class MerkleBlockHandlerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        sampleMerkleBlockMessage = MerkleBlockMessage(blockHeaderItem: blockHeaderItem, totalTransactions: 1, numberOfHashes: 1, hashes: [Data()], numberOfFlags: 0, flags: [])
    ...
    func testValidMerkleBlock() {
        ...
        stub(mockValidator) { mock in
            when(mock.isValid(message: equal(to: sampleMerkleBlockMessage))).thenReturn(true)
        }
    ...
    func testInvalidMerkleBlocks() {
        ...
        stub(mockValidator) { mock in
            when(mock.isValid(message: equal(to: sampleMerkleBlockMessage))).thenReturn(false)
        }
